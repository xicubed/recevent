...
(def group-id 'edu.uidaho.eod)
(def artifact-id 'recevent)
(def artifact 'edu.uidaho.eod/recevent)

(def main-ns 'edu.uidaho.eod.recevent)

(def target-folder "target")
(def compile-java-sources? false)
(def java-src-folders ["java-src"])
(def release-branches #{"main"})
(def deployable-branches #{"main" "develop"})
