...
(def project-env {;:group-id              (System/getenv "group-id")
                  :group-id              (clojure.edn/read-string (str "edu.uidaho.eod"))
                  ;:artifact-id           (System/getenv "artifact-id")
                  :artifact-id           (clojure.edn/read-string (str "recevent"))
                  ;:artifact              (symbol (System/getenv "artifact"))
                  :artifact              (clojure.edn/read-string (str "edu.uidaho.eod/recevent"))
                  ;:main-ns               (symbol (System/getenv "main-ns"))
                  :main-ns               (clojure.edn/read-string (str "edu.uidaho.eod.recevent"))
...
; this one lets u use a template pom
#_(defn uberjar
    "Build the Uberjar."
    [opts]
    (-> opts
